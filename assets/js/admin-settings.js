/**
 * Listing Moderation for HivePress: settings screen behaviour.
 *
 * Everything between the SHARED SETTINGS CHROME banner and the end of
 * addSettingsChrome() is copied from the reference implementation in Account
 * Menu Enhancer for HivePress (assets/js/backend.js). Only the two constants
 * in CHROME and the name of the localised data object differ. Do not "improve"
 * it here: fix the reference and sweep every sibling in the same session
...
( function () {
	...
	function chromeLabels() {
		return ( window.hpalmSettingsData && window.hpalmSettingsData.labels ) || {};
	}
	...
	/**
	 * This plugin's own settings-screen work.
	 *
	 * The stylesheet's non-chrome rules are scoped by .hpalm-settings, so the
	 * class goes on as soon as the form is known to be ours. chromeForm()
	 * already answers that question with the same test the CSS depends on, so
	 * the class is added from its answer rather than from a second selector
	...
	function addSettingsScope() {
		...
		if ( form ) {
			form.classList.add( 'hpalm-settings' );
		}
	}
