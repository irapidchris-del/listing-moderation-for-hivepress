 ...
 */
const HPALM_UPDATE_REASON_KEY = 'hpalm_github_release_reason';
 ...
 */
const HPALM_UPDATE_RATE_LIMIT_KEY = 'hpalm_github_release_rate_limit';
...
function hpalm_get_latest_release( $force = false ) {
	$cached = get_site_transient( HPALM_UPDATE_CACHE_KEY );
	...
	if ( ! $release && $cached ) {
		set_site_transient( HPALM_UPDATE_CACHE_KEY, $cached, HOUR_IN_SECONDS );
 ...
 */
function hpalm_fetch_release_data() {
	$site = hpalm_fetch_release_from_site();

	if ( isset( $site['release'] ) ) {
		delete_site_transient( HPALM_UPDATE_REASON_KEY );
	...
	if ( isset( $site['reason'] ) && 'no_release' === $site['reason'] ) {
		set_site_transient( HPALM_UPDATE_REASON_KEY, 'no_release', HOUR_IN_SECONDS );

		return null;
	}

	return hpalm_fetch_release_from_api();
}
 ...
 *                              back to the API.
 */
function hpalm_fetch_release_from_site() {
	$base = 'https://github.com/' . HPALM_UPDATE_REPO;

	$response = hpalm_request(
	...
	$asset = hpalm_fetch_release_asset( $base, $tag );
	...
	$notes = hpalm_fetch_release_notes( $base, $tag );
 ...
 * @return array<string, string>|null
 */
function hpalm_fetch_release_asset( $base, $tag ) {
	$response = hpalm_request( $base . '/releases/expanded_assets/' . rawurlencode( $tag ) );
 ...
 * @return array<string, string>
 */
function hpalm_fetch_release_notes( $base, $tag ) {
	$empty = [
		'body'      => '',
		'published' => '',
	];

	$response = hpalm_request( $base . '/releases.atom' );
	...
	foreach ( $entries[1] as $entry ) {
		...
		if ( preg_match( '#<content[^>]*>(.*?)</content>#s', $entry, $content ) ) {
			$notes = hpalm_release_notes_to_text( $content[1] );
		}
 ...
 * @return string
 */
function hpalm_release_notes_to_text( $html ) {
 ...
 *
 * Kept as a fallback only. See `hpalm_fetch_release_data()` for why it is not the first choice.
 *
 * @return array<string, mixed>|null
 */
function hpalm_fetch_release_from_api() {
	...
	if ( get_site_transient( HPALM_UPDATE_RATE_LIMIT_KEY ) ) {
		set_site_transient( HPALM_UPDATE_REASON_KEY, 'rate_limited', HOUR_IN_SECONDS );
	...
	if ( is_wp_error( $response ) ) {
		set_site_transient( HPALM_UPDATE_REASON_KEY, 'unreachable', HOUR_IN_SECONDS );
	...
	if ( 200 !== $code ) {
		...
		if ( ( 403 === $code || 429 === $code ) && '0' === (string) wp_remote_retrieve_header( $response, 'x-ratelimit-remaining' ) ) {
			...
			set_site_transient( HPALM_UPDATE_RATE_LIMIT_KEY, $reset ? $reset : time() + $wait, $wait );
		}

		set_site_transient( HPALM_UPDATE_REASON_KEY, $reason, HOUR_IN_SECONDS );

		return null;
	}

	delete_site_transient( HPALM_UPDATE_RATE_LIMIT_KEY );
	delete_site_transient( HPALM_UPDATE_REASON_KEY );
 ...
 * @return array<string, mixed>|null
 */
function hpalm_request( $url, $args = [] ) {
	$response = wp_remote_get(
		$url,
		array_merge(
			[
...
function hpalm_handle_update_check() {
	...
	$reason = get_site_transient( HPALM_UPDATE_REASON_KEY );
	...
	if ( 'no_release' === $reason ) {
		...
		$status = 'error';
	} elseif ( $release && version_compare( $release['version'], hpalm_get_version(), '>' ) ) {
		$status = 'available';
	} else {
		$status = 'none';
	}
...
function hpalm_show_update_check_notice() {
	...
	if ( 'available' === $status ) {
		...
		$message = esc_html__( 'No releases have been published for Automated Listing Moderation for HivePress yet, so there is nothing to update to. This is normal for a brand new copy and does not mean anything is wrong.', 'listing-moderation-for-hivepress' );
		$class   = 'notice-info';
	} elseif ( 'limited' === $status ) {
		$message = esc_html__( 'GitHub limits how many update checks one server may make each hour, and this server has reached that limit. Nothing is wrong with the plugin or your site, and checking will work again within the hour.', 'listing-moderation-for-hivepress' );
 ...
 * @return object|false
 */
function hpalm_inject_update( $transient ) {
	if ( ! is_object( $transient ) ) {
		return $transient;
	}

	$basename = plugin_basename( __FILE__ );
	$release  = get_site_transient( HPALM_UPDATE_CACHE_KEY );
	$version  = hpalm_get_version();
 ...
 * @return array<string, string>
 */
function hpalm_add_bulk_check( $actions ) {
	if ( current_user_can( 'update_plugins' ) ) {
		$actions['hpx_check_updates'] = __( 'Check for updates', 'listing-moderation-for-hivepress' );
	}
 ...
 * @return string
 */
function hpalm_handle_bulk_check( $redirect, $action, $plugin_files ) {
 ...
 * @return string
 */
function hpalm_finish_bulk_check( $redirect, $action, $plugin_files ) {
 ...
 * @return void
 */
function hpalm_show_bulk_check_notice() {
	...
	if ( $available ) {
		...
		$message = sprintf( _n( 'Checked %1$s plugin for updates: %2$s can be updated.', 'Checked %1$s plugins for updates: %2$s can be updated.', $checked, 'listing-moderation-for-hivepress' ), number_format_i18n( $checked ), number_format_i18n( $available ) );
	} else {
		/* translators: %s: number of plugins checked. */
		$message = sprintf( _n( 'Checked %s plugin for updates: it is up to date.', 'Checked %s plugins for updates: all are up to date.', $checked, 'listing-moderation-for-hivepress' ), number_format_i18n( $checked ) );
	}
 ...
 * @return void
 */
function hpalm_print_plugins_screen_styles() {
...
add_filter( 'site_transient_update_plugins', 'hpalm_inject_update' );
add_filter( 'handle_bulk_actions-plugins', 'hpalm_handle_bulk_check', 10, 3 );
...
if ( empty( $GLOBALS['hpx_update_check_bulk'] ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
	$GLOBALS['hpx_update_check_bulk'] = 'listing-moderation-for-hivepress'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.

	add_filter( 'bulk_actions-plugins', 'hpalm_add_bulk_check' );
	add_filter( 'handle_bulk_actions-plugins', 'hpalm_finish_bulk_check', 20, 3 );
	add_action( 'admin_notices', 'hpalm_show_bulk_check_notice' );
	add_action( 'network_admin_notices', 'hpalm_show_bulk_check_notice' );
	add_action( 'admin_print_styles-plugins.php', 'hpalm_print_plugins_screen_styles' );
}
